        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            try
            {
                while (!stoppingToken.IsCancellationRequested)
                {
                    ...
                    _logger.LogInformation("Inicio en hostname: {hostName}", hostName);
                    try
                    {
                    ...
                    try
                    {
                        ...
                        if (reader.Read())
                        {
                            ...
                            CheckBDDTO checkBDDTO = new CheckBDDTO
                            {
                                ...
                                Nombre_servidor = hostName,
                                ServidorType = ServidorType.Contenedor,
                                Nombre_BD = "BD SQL server Visualab",
                                Respuesta_data = "Ok"
                            };
                    ...
                    catch (Exception ex)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                var message = await httpResponseBD.GetErrorMessageAsync();
                                _logger.LogError($"Error al envío de CHECK BD de SQL server {message}");
                            }
                        }
                        catch (Exception exp)
                        {
                    ...
                    try
                    {
                        ...
                        if (reader.Read())
                        {
                            ...
                            CheckBDDTO checkBDDTO = new CheckBDDTO
                            {
                                ...
                                Nombre_servidor = hostName,
                                ServidorType = ServidorType.Contenedor,
                                Nombre_BD = "BD PostgreSQL Visualab",
                                Respuesta_data = "Ok"
                            };
                            var httpResponseBD = await _repository.Post("/api/monitores/bd", checkBDDTO);
                            if (httpResponseBD.Error)
                            {
                                var message = await httpResponseBD.GetErrorMessageAsync();
                                _logger.LogError($"Error al envío de CHECK BD PostgreSQL {message}");
                            }
                        }
                        else
                        {
